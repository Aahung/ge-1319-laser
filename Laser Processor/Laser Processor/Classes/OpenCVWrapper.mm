...
+ (float)microShiftInnerProductGPU: (NSArray *)baseImagePixels imagePixels: (UInt8 *)imagePixels imageRow: (int)imageRow imageCol: (int)imageCol maxShift: (int)maxShift {
    ...
    for (int i = 0; i < imageRow; ++i) {
        for (int j = 0; j < imageCol; ++j) {
            ...
            int index4 = 4 * index;
            baseImage.at<uchar>(i, j) = (uchar)[baseImagePixels[index] integerValue];
